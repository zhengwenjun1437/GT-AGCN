...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            logger.info \
                ("token_vocab: {}, post_vocab: {}, pos_vocab: {}, dep_vocab: {}, pol_vocab: {}".format(len(token_vocab), len(post_vocab), len(pos_vocab), len(dep_vocab), len(pol_vocab)))
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    if test_acc > max_test_acc:
                        ...
                        if test_acc > max_test_acc_overall:
                            if not os.path.exists('./state_dict_twitter'):
                                os.mkdir('./state_dict_twitter')
                            model_path = './state_dict_twitter/{}_{}_acc_{:.4f}_f1_{:.4f}'.format(self.opt.model_name, self.opt.dataset, test_acc, f1)
    ...
    def run(self):
        ...
        return max_test_acc_overall, max_f1_overall, model_path
...
def main():
    ...
    input_colses = {
        'non-bert': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length', 'adj', 'adj_dis'],
    ...
    parser.add_argument('--alpha', default=0.7, type=float, help='the weight of distance')
    ...
    param_grid = {
        ...
        'ffn_dropout': [0.3],
        'alpha': [0.9],
        # 'attention_heads': [1, 2, 3, 4, 5, 6, 10, 12],
        # 'attn_dropout': [0.1, 0.3, 0.5, 0.7],
        # 'hidden_dim': [48, 72, 120, 240, 360],
    }
    ...
    for i in range(MAX_EVALS):
        # 记录到日志
        if i % 10 == 0:
            if not os.path.exists('./logs_search'):
                os.makedirs('./logs_search', mode=0o777)
            log_file = '{}-{}-{}.log'.format(opt.model_name, opt.dataset, strftime("%Y-%m-%d_%H-%M-%S", localtime()))
            logger.addHandler(logging.FileHandler("%s/%s" % ('./logs_search', log_file)))

        logger.info("")
        logger.info(">> 这是第 {} 次超参数搜索......".format(i))

        random.seed(i)
        hyperparameters = {k: random.sample(v, 1)[0] for k, v in param_grid.items()}

        # opt.learning_rate = hyperparameters["learning_rate"]
        # opt.batch_size = hyperparameters["batch_size"]
        opt.num_layers = hyperparameters["num_layers"]
        opt.gcn_layers = hyperparameters["gcn_layers"]
        # opt.input_dropout = hyperparameters["input_dropout"]
        # opt.ffn_dropout = hyperparameters["ffn_dropout"]
        opt.alpha = hyperparameters["alpha"]
        # opt.attention_heads = hyperparameters["attention_heads"]
        # opt.attn_dropout = hyperparameters["attn_dropout"]
        # opt.hidden_dim = hyperparameters["hidden_dim"]

        ins = Instructor(opt)
        max_test_acc_overall, max_f1_overall, model_path = ins.run()

        if max_test_acc_overall > best_score:
            best_hyperparams = hyperparameters
            best_score = max_test_acc_overall

            logger.info("><" * 30)
            logger.info(">> best_hyperparams: {}".format(best_hyperparams))
            logger.info(">> best_model_path: {}".format(model_path))
            logger.info("><" * 30)
            logger.info("")
    ...
    logger.info(">> Best Hyperparams Is: {}".format(best_hyperparams))


if __name__ == '__main__':
    main()

# python train_search.py --model_name text-gt --scheduler linear --warmup 2  --balance_loss --use_rnn