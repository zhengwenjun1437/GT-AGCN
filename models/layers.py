...
class TextGINConv(nn.Module):
    ...
    def forward(self, x, adj, e):  # x is (B, L, F), adj is (B, L, L), e is (B, L, L, Fe)
        """
        x:      torch.Size([16, 41, 60])
        adj:    torch.Size([16, 41, 41])
        e:      torch.Size([16, 41, 41, 30])
        """
        ...
        out = self.ffn(out + x)
        # print(">> out:", out.shape)     # out: torch.Size([16, 41, 60])
...
class TextGATConv(nn.Module):
    ...
    def forward(self, x, adj, e):  # x is (B, L, F), adj is (B, L, L), e is (B, L, L, Fe)
        ...
        out = torch.bmm(alpha.contiguous().view(-1, L, L), x.contiguous().view(-1, L, C)).view(B, H, L,
                                                                                               C)  # (B, H, L, L), (B, H, L, C) -> (B, H, L, C)
