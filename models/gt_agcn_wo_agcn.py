...
class GT_AGCN_WO_AGCN(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.post_emb = nn.Embedding(2 * opt.max_position + 2, opt.post_dim, padding_idx=0) if opt.post_dim > 0 else None
        self.deprel_emb = nn.Embedding(opt.deprel_size + 1, opt.deprel_dim, padding_idx=0) if opt.deprel_dim > 0 else None
        in_dim = opt.embed_dim + opt.pos_dim + opt.post_dim

        if opt.use_rnn:
            self.rnn = nn.LSTM(in_dim, opt.rnn_hidden, batch_first=True, dropout=opt.rnn_dropout if opt.rnn_layers > 1 else 0.0, bidirectional=opt.bidirect)
    ...
    def forward(self, inputs):
        tok, asp, pos, head, deprel, post, mask, length, adj, adj_dis = inputs
        ...
        # adj_dis = adj_dis[:, :maxlen, :maxlen]
        # adj_dis = torch.exp(self.opt.alpha * (-1.0) * adj_dis).type(torch.float32)
        ...
        #     h_dis = h_dis / (adj_dis.sum(-1).unsqueeze(-1) + 1)
        #     h_dis = F.relu(h_dis)

        # h1 = adj_dis.bmm(h_dis)
        # h1 = self.wa_1(h1)
        # h1 = h1 / (adj_dis.sum(-1).unsqueeze(-1) + 1)
        # h1 = F.relu(h1)
        #
        # h2 = adj_dis.bmm(h1)
        # h2 = self.wa_2(h2)
        # h2 = h2 / (adj_dis.sum(-1).unsqueeze(-1) + 1)
        # h2 = F.relu(h2)
