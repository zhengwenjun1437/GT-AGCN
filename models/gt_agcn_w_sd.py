...
class GT_AGCN_W_SD(nn.Module):
    ...
    def forward(self, inputs):
        ...
        adj_3 = adj_3[:, :maxlen, :maxlen]
        adj_3 = torch.exp(self.opt.alpha * (-1.0) * adj_3).type(torch.float32)
        src_mask = (tok != 0)
        ...
        for j in range(self.opt.gcn_layers):
            h_dis = adj_3.bmm(h_dis)
            h_dis = self.wa_s[j](h_dis)
            h_dis = h_dis / (adj_3.sum(-1).unsqueeze(-1) + 1)
            h_dis = F.relu(h_dis)
